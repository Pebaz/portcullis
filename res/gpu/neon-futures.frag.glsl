/*
Author: Drakyen https://www.shadertoy.com/view/7lXfz7
*/
...
vec3 ray(vec3 p, vec3 l, vec2 the_uv, float z) {
    vec3 f = normalize(l-p),
        r = normalize(cross(vec3(0,1,0), f)),
        u = cross(f,r),
        c = f*z,
        i = c + the_uv.x*r + the_uv.y*u,
        d = normalize(i);
    return d;
}
...
{
  the_uv=vec2(frag.x/res.x,frag.y/res.y);
  the_uv-=0.5;the_uv/=vec2(res.y/res.x,1);
	tt=mod(time,300.);
  vec3 ro=rp=cam(),rd=ray(ro,look(ro),the_uv, 1.);
